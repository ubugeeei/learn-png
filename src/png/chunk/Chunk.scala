...
/** A validated four-letter PNG chunk type.
  ...
  * [[https://www.w3.org/TR/png-3/#5Chunk-naming-conventions PNG §5.4]]. The
  * third (reserved) letter must currently be uppercase. Unknown critical chunks
  * cannot safely be ignored by a decoder.
  */
...
object ChunkType:
  ...
  def fromString(value: String): Either[PngError, ChunkType] =
    if value.length != 4 then
      Left(InvalidChunkType("must contain exactly four letters"))
    else if !value.forall(character =>
        character >= 'A' && character <= 'Z' || character >= 'a' && character <= 'z'
      )
    then Left(InvalidChunkType("characters must be ASCII letters"))
    else if value(2).isLower then
      Left(InvalidChunkType("the reserved third letter must be uppercase"))
    else Right(value)
...
/** One physical PNG chunk, with a validated type and immutable payload.
  *
  * The serialized layout and CRC coverage follow
  * [[https://www.w3.org/TR/png-3/#5Chunk-layout PNG §5.3]].
  */
final class Chunk private (
  ...
  override def equals(other: Any): Boolean = other match
    case that: Chunk =>
      chunkType == that.chunkType && java.util.Arrays
        .equals(payload, that.payload)
    case _ => false

  override def hashCode(): Int =
    31 * chunkType.hashCode + java.util.Arrays.hashCode(payload)
  override def toString: String =
    s"Chunk(${chunkType.name}, ${payload.length} bytes)"

object Chunk:
  private val MaxLength = Int.MaxValue.toLong

  def apply(chunkType: ChunkType, data: Array[Byte]): Either[PngError, Chunk] =
    if data.length.toLong > MaxLength then
      Left(InvalidChunkLength(chunkType.name, data.length))
  ...
  def parse(cursor: Binary.Cursor): Either[PngError, Chunk] =
    for
      ...
      _ <- Either.cond(
        length <= MaxLength,
        (),
        InvalidChunkLength("unknown", length)
      )
      rawType <- cursor.take(4)
      kind <- ChunkType.fromString(
        new String(rawType, StandardCharsets.US_ASCII)
      )
      ...
      _ <- Either.cond(
        expected == actual,
        (),
        CrcMismatch(kind.name, expected, actual)
      )
