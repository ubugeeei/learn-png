...
/** The five adaptive scanline filters from [[https://www.w3.org/TR/png-3/#9Filters PNG §9]].
  *
  * Arithmetic is modulo 256. `bytesPerPixel` is the specification's `bpp`: at least one byte, rounded up for
  * sub-byte samples.
  */
private[png] enum Filter(val code: Int):
  ...
  def encode(row: Array[Byte], previous: Array[Byte], bytesPerPixel: Int): Array[Byte] =
    Array.tabulate(row.length): index =>
      val left =
        if index >= bytesPerPixel then row(index - bytesPerPixel) & 0xff
        else 0
      val up =
        if previous.nonEmpty then previous(index) & 0xff
        else 0
      val upperLeft =
        if previous.nonEmpty && index >= bytesPerPixel then previous(index - bytesPerPixel) & 0xff
        else 0
      val predictor =
        this match
          case None =>
          ...
          case Average =>
            (left + up) / 2
          case Paeth =>
            Filter.paeth(left, up, upperLeft)
      ((row(index) & 0xff) - predictor).toByte

  def decode(filtered: Array[Byte], previous: Array[Byte], bytesPerPixel: Int): Array[Byte] =
...
object Filter:
  def fromCode(code: Int): Either[PngError, Filter] = Filter.values
    .find(_.code == code)
    .toRight(InvalidFilter(code))

  def choose(row: Array[Byte], previous: Array[Byte], bytesPerPixel: Int): (Filter, Array[Byte]) =
    Filter.values
