...
/** Executable entry point for inspecting and normalizing PNG files.
  *
  * Run `scala-cli run . -- info image.png` or `scala-cli run . -- copy input.png output.png --interlace`.
  */
object PngCli:
  ...
  private def parseOptions(flags: List[String]): Either[PngError, EncoderOptions] =
    flags.foldLeft[Either[PngError, EncoderOptions]](Right(EncoderOptions.default)):
      case (result, "--interlace") =>
        result.flatMap(options => EncoderOptions(true, options.compressionLevel, options.maximumIdatPayload))
      case (_, unknown) =>
