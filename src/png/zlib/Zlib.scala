...
/** The zlib transport required for IDAT by
  * [[https://www.w3.org/TR/png-3/#10Compression PNG §10]].
  */
private[png] object Zlib:
  def compress(
    ...
    Try:
      ...
      Using.resource(DeflaterOutputStream(output, Deflater(level)))(
        _.write(input)
      )
      output.toByteArray
  ...
  private final class BoundedOutputStream(limit: Int)
    ...
    private def ensureCapacity(additional: Int): Unit =
      if output.size().toLong + additional > limit then
        throw IllegalArgumentException(
          s"decompressed data exceeds $limit bytes"
        )
