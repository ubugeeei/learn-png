...
/** PNG color types and their permitted sample depths.
  *
  * The numeric values and valid combinations come from
  * [[https://www.w3.org/TR/png-3/#11IHDR PNG §11.2.2]].
  */
enum ColorType(val code: Int, val channels: Int, val bitDepths: Set[Int])
    derives CanEqual:
...
object ColorType:
  def fromCode(code: Int): Either[PngError, ColorType] =
    ColorType.values
      .find(_.code == code)
...
final case class Header private (
  ...
  private[png] def bytes: Array[Byte] =
    width.toLong.uint32Bytes ++ height.toLong.uint32Bytes ++ Array(
      bitDepth.toByte,
      colorType.code.toByte,
      0.toByte,
      0.toByte,
      (if interlaced then 1 else 0).toByte
    )
...
object Header:
  def apply(
    ...
    if width <= 0 || height <= 0 then
      Left(InvalidHeader("width and height must be positive"))
    else if !colorType.bitDepths(bitDepth) then
      Left(
        InvalidHeader(
          s"bit depth $bitDepth is invalid for color type ${colorType.code}"
        )
      )
    else if width.toLong * bitDepth * colorType.channels > Int.MaxValue.toLong * 8
    then Left(InvalidHeader("scanline is too large"))
  ...
  private[png] def parse(data: Array[Byte]): Either[PngError, Header] =
    if data.length != 13 then
      Left(InvalidHeader(s"length must be 13, found ${data.length}"))
    else
      ...
      for
        ...
        _ <- Either.cond(
          width <= Int.MaxValue,
          (),
          InvalidHeader("width exceeds JVM limits")
        )
        _ <- Either.cond(
          height <= Int.MaxValue,
          (),
          InvalidHeader("height exceeds JVM limits")
        )
        _ <- Either.cond(
          compression == 0,
          (),
          InvalidHeader(s"compression method $compression")
        )
        _ <- Either.cond(
          filter == 0,
          (),
          InvalidHeader(s"filter method $filter")
        )
        _ <- Either.cond(
          interlace == 0 || interlace == 1,
          (),
          InvalidHeader(s"interlace method $interlace")
        )
